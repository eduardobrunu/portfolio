...
import { getSocials, SocialItem } from './socialData';
...
export default function SocialLinks({
  ...
  const items = getSocials(keys);

  const baseItem = 'group flex items-center justify-center border border-slate-700 bg-slate-800/50 backdrop-blur-sm transition-all duration-300 hover:bg-slate-800/70 hover:shadow-md hover:shadow-black/20 hover:ring-2';
      ...
      {items.map((s, i) => {
        ...
        return (
          <motion.a
            ...
            whileTap={{ scale: 0.95 }}
            className={`${baseItem} ${sizeByVariant[variant]} ${s.ringClass} ${itemClassName}`.trim()}
          >
            <Icon className={`w-5 h-5 ${s.brandClass}`} />
            {showLabels && (
